"""E16 rank-16 top-8 max832/max1024 output-length grid on dev-200."""
...
from uit_dsc_fixed_rag.e14_rank16_dev200 import _adapter_evidence
from uit_dsc_fixed_rag.e14_rank16_lora import (
    E14Config,
    load_config as load_training_config,
)
from uit_dsc_fixed_rag.e15_context_count_grid import (
    E15Config,
    _metrics,
    _paired_delta,
)
...
class E16Error(RuntimeError):
    """Raised when E16 evidence or checkpoint identity changes."""


def load_config(path: Path) -> E16Config:
    ...
    if (
        ...
        raise ValueError("E16 config root is incompatible.")
    config = E16Config(payload, path)
    ...
    if (
        ...
        raise ValueError("E16 source context identity changed.")
    ...
    if (
        ...
        or control.get("variant") != "rank16_top8_max704"
        or control.get("context_limit") != 8
        ...
        raise ValueError("E16 E15 top-8 max704 control identity changed.")
    ...
    if (
        ...
        raise ValueError("E16 rank-16 training identity changed.")
    expected_variants = [
        {
            "key": "rank16_top8_max832", "max_new_tokens": 832,
            "worker_rank": 0, "device": "cuda:0",
        },
        {
            "key": "rank16_top8_max1024", "max_new_tokens": 1024,
    ...
    if (
        ...
        or inference.get("context_limit") != 8
        or inference.get("control_max_new_tokens") != 704
        ...
        raise ValueError("E16 output-length grid changed.")
    execution = config.section("execution")
    if execution != {
        "workers": 2,
        "partition": "one-output-length-variant-per-worker-all-dev200-ids",
        "questions_per_worker": 200,
        "checkpoint_after_questions": 1,
    }:
        raise ValueError("E16 dual-variant execution changed.")
    ...
    if (
        ...
        raise ValueError("E16 parameter budget failed.")
    if config.section("scoring").get("promotion_allowed") is not False:
        raise ValueError("E16 dev-200 cannot auto-promote.")
    contract = config.section("run_contract")
    if not contract or not all(value is True for value in contract.values()):
        raise ValueError("E16 run contract lost an invariant.")
    return config


def variant_for_worker(config: E16Config, worker_rank: int) -> dict[str, Any]:
    matches = [item for item in config.variants if item["worker_rank"] == worker_rank]
    if len(matches) != 1:
        raise E16Error(f"Invalid E16 worker rank: {worker_rank}")
    return matches[0]


def assigned_indices(config: E16Config, worker_rank: int) -> list[int]:
    variant_for_worker(config, worker_rank)
    return list(range(config.section("dev")["sample_size"]))


def _training_config(project_root: Path, config: E16Config) -> E14Config:
    ...
    if not path.is_file() or file_sha256(path) != section["config_sha256"]:
        raise E16Error("Pinned E14 training config changed.")
    ...
    if (
        ...
        raise E16Error("E16 prompt changed from E14 training.")
    return training


def _control_rows(
    control_directory: Path, dev_path: Path, config: E16Config,
) -> list[dict[str, Any]]:
    ...
    if not path.is_file() or file_sha256(path) != section["sha256"]:
        raise E16Error("Saved E15 top-8 max704 results changed.")
    _, ids = _sample_ids(dev_path, config)
    rows = _read_jsonl(path)
    if len(rows) != len(ids):
        raise E16Error("Saved E15 top-8 max704 result count changed.")
    for index, (question_id, row) in enumerate(zip(ids, rows)):
        ...
        if (
            ...
            or variant.get("context_limit") != 8
            or variant.get("max_new_tokens") != 704
            or not isinstance(variant.get("output_tokens"), int)
            or variant.get("finish_reason") not in {"eos", "length", "other"}
        ):
            raise E16Error(f"Invalid E15 top-8 max704 control row: {index}")
    report_path = control_directory / "report.json"
    if not report_path.is_file():
        raise E16Error("Saved E15 report is missing.")
    ...
    if (
        ...
        raise E16Error("Saved E15 report identity changed.")
    return rows


def validate_preflight(
    *, project_root: Path, contexts_directory: Path, control_directory: Path,
    training_directory: Path, dev_path: Path, config: E16Config,
) -> dict[str, Any]:
    ...
    if (
        ...
        raise E16Error("Pinned official scorer changed.")
    ...
    if adapter["adapter_sha256"] != config.section("source_control")["adapter_sha256"]:
        raise E16Error("E16 control and generator adapter hashes differ.")
    ...
    return {
        ...
        "max_new_tokens": [item["max_new_tokens"] for item in config.variants],
    }


def load_generator(
    *, project_root: Path, training_directory: Path,
    config: E16Config, device: str,
    ...
    try:
        ...
        raise E16Error(str(exc)) from exc
...
def run_variant_worker(
    *, worker_rank: int, device: str, model: Any, tokenizer: Any,
    ...
    config: E16Config, device_map: dict[str, Any], adapter_parameters: int,
) -> dict[str, Any]:
    variant = variant_for_worker(config, worker_rank)
    if device != variant["device"]:
        raise E16Error("E16 worker-to-device mapping changed.")
    contexts = _context_rows(contexts_directory, dev_path, config)
    ...
    identity = {
        "code_version": CODE_VERSION,
        "stage": "e16-rank16-top8-output-length-grid-dev200-worker",
        "config_sha256": config.config_sha256,
    ...
    for position in range(completed, len(assigned)):
        ...
        candidates = contexts[index]["contexts"][:8]
        if len(candidates) != 8:
            raise E16Error(f"Top-8 context prefix is incomplete: {question_id}")
        ...
        if not answer:
            raise E16Error(f"Empty E16 answer: {question_id}")
        ...
        LOGGER.info(
            "e16_generation_progress variant=%s device=%s completed=%d total=%d "
...
def finalize_score(
    *, contexts_directory: Path, control_directory: Path,
    ...
    contexts = _context_rows(contexts_directory, dev_path, config)
    controls = _control_rows(control_directory, dev_path, config)
    control_key = "rank16_top8_max704_control"
    ...
    for index in range(len(ids)):
        ...
        control["answer_source"] = "reused-byte-verified-e15-top8-max704-control"
        rows_by_key[control_key].append(control)
    for variant in config.variants:
        root = output_directory / "evaluation" / variant["key"]
        state_path = root / "state.json"
        if not state_path.is_file():
            raise E16Error(f"Missing E16 state: {variant['key']}")
        ...
        if (
            ...
            or state_identity.get("adapter_sha256") != source_control["adapter_sha256"]
            or state_identity.get("variant") != variant["key"]
            or state_identity.get("context_limit") != 8
            or state_identity.get("max_new_tokens") != variant["max_new_tokens"]
        ):
            raise E16Error(f"Incomplete or mismatched E16 worker: {variant['key']}")
        for index, question_id in enumerate(ids):
            path = root / "records" / f"{index:04d}.json"
            if not path.is_file():
                raise E16Error(f"Missing E16 record: {variant['key']}:{index}")
            row = json.loads(path.read_text(encoding="utf-8"))
            expected_prefix = [
                item["chunk_id"] for item in contexts[index]["contexts"][:8]
            ]
            if (
                row.get("question_id") != question_id
                or row.get("sample_index") != index
                ...
                raise E16Error(f"Invalid E16 record: {variant['key']}:{index}")
    ...
    if (
        ...
        raise E16Error("Re-scored E15 top-8 max704 control changed.")
    paired_vs_control = {
        key: _paired_delta(
            scored[key], scored[control_key], config=config,
            label=f"{key}-minus-max704",
        )
        for key in keys[1:]
    }
    paired_1024_minus_832 = _paired_delta(
        scored["rank16_top8_max1024"],
        scored["rank16_top8_max832"],
        config=config,
        label="max1024-minus-max832",
    )
    ...
    report = {
        ...
        "only_changed_factor": "max_new_tokens: 704 control vs 832/1024",
        "fixed_context_limit": 8,
        "metrics": metrics,
        "paired_deltas_vs_max704_control": paired_vs_control,
        "paired_delta_max1024_minus_max832": paired_1024_minus_832,
        "smoke_leader": leader,
        ...
        "warning": (
            "E16 is a dev-200 tuning grid on a repeatedly used selection sample. "
...
__all__ = [
    "E16Config", "E16Error", "assigned_indices", "finalize_score",
